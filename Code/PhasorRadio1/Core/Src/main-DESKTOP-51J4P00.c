...
/* USER CODE BEGIN PV */
	uint8_t rotaryEncoderKey;
	int8_t rotaryEncoderStep;
	uint8_t rotaryEncoderEdgeCount;
	...
	//config SI5351
	uint8_t phaseOffset;
	const int32_t correction5351 = 116800;
...
//		uint8_t bandValue;
//	};
//	struct FreqBand queueUpdateValue;

	uint8_t swapIQ = 0;
	uint8_t swapIQhasChanged = 0;

	uint16_t rotaryEncoderStepSize = 1000;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  {
	if(rotaryEncoderStepSize == 1000)
		{
			rotaryEncoderStepSize = 100;
	...
	else if(rotaryEncoderStepSize == 100)
		{
			rotaryEncoderStepSize = 10;
	...
	else if(rotaryEncoderStepSize == 10)
		{
			rotaryEncoderStepSize = 1000;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	if(rotaryEncoderEdgeCount >= 2)
	{
		newVFOFrequency();
		rotaryEncoderEdgeCount = 0;
	}
}
...
void newVFOFrequency()
{
	...
	{
		vfo_40m_5351 = vfo_40m_5351 + rotaryEncoderStepSize*rotaryEncoderStep;
		vfo_40m = (float)vfo_40m_5351/1000.0;
		VFOhasChanged = TRUE;
	}
}
