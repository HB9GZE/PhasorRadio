...
/* Definitions for myTaskSI5351 */
osThreadId_t myTaskSI5351Handle;
const osThreadAttr_t myTaskSI5351_attributes =
{ .name = "myTaskSI5351", .stack_size = 1024 * 4, .priority = (osPriority_t) osPriorityAboveNormal, };
...
void StartTaskSI5351(void *argument);
...
int main(void)
{
	...
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_14, GPIO_PIN_SET); //set input bandpass to 40m
	HAL_GPIO_WritePin(GPIOG, GPIO_PIN_4, GPIO_PIN_RESET); //set input bandpass to 40m
	HAL_GPIO_WritePin(GPIOG, GPIO_PIN_5, GPIO_PIN_SET); //LSB
...
//si5351_SetupOutput(2, SI5351_PLL_A, SI5351_DRIVE_STRENGTH_4MA, &outConf,phaseOffset);
	...
	/* creation of myTaskSI5351 */
	myTaskSI5351Handle = osThreadNew(StartTaskSI5351, NULL, &myTaskSI5351_attributes);
...
void StartTaskFromGui(void *argument)
{
	...
	for (;;)
	{
		...
		if (stateLSBUSB != lastStateLSBUSB)
		{
			...
			{
				HAL_GPIO_WritePin(GPIOG, GPIO_PIN_5, GPIO_PIN_RESET); //USB
			}
...
/* USER CODE BEGIN Header_StartTaskSI5351 */
/**
 * @brief Function implementing the myTaskSI535 thread.
 * @param argument: Not used
 * @retval None
 */
/* USER CODE END Header_StartTaskSI5351 */
void StartTaskSI5351(void *argument)
{
	/* USER CODE BEGIN StartTaskSI5351 */
	/* Infinite loop */
	for (;;)
	{
		if (startTimerA == TRUE)
		{
			osTimerStart(myTimerI2CWindowAHandle, 2);
			startTimerA = FALSE;
		}
		else
		{
